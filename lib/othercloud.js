...
//        req.setRequestHeader('X-Auth-Token', token);
    ...
    var createQcloudKeypair = function (keyName,regionId,callback,callbackError) {
        var body = {
            regionId:regionId,
            keyName:keyName
        }
        ...
        sendRequest('POST','hybrid/qcloud/keypair',JSON.stringify(body),function(resp){
            console.log('BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB');
            console.log(resp);
          callback(JSON.parse(resp));
        },callbackError);

        //send('POST','hybrid/qcloud/keypair',JSON.stringify(body),undefined,callback,callbackError);
